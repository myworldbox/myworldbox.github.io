...
class CurrentFunc {
  List<Widget> _wrapForm(MwbAuthRegister app) {
  ...
  Widget _form(MwbAuthRegister app) {
    final selector = app.local.utility._utilitySelector;
                  ...
                  TextSpan(
                    ...
                    recognizer: TapGestureRecognizer()
                      ..onTap = () {
                        launchUrl(
                          Uri.parse(
                            "${CoreStatic.coreConst.url.host.myapibox}/api/file?name=teams_and_services",
                          ),
                        );
                      },
                  ),
  ...
  List<Widget> _layout(MwbAuthRegister app) {
    final selector = app.local.utility._utilitySelector;
...
class MwbAuthRegister
    extends
        ModelApp<
          Pager<MwbAuthRegister>,
          CurrentParam,
          CurrentUtility,
          CurrentFunc
        > {
      ...
      param
        ...
        .._requestLogin = [
          ...
          (
            ...
            headers: headers,
            body: () => {
              "operator": {"provider": "google_sheet", "action": "exist"},
              "cache": {
              ...
              "data": {
                CoreEnumInput.mailSchool.name:
                    param._data![CoreEnumInput.mailSchool.name]!,
              },
            },
            callback: (Response result) async {
              switch (result.statusCode) {
                case 200:
                  if (result.body == "false") {
                    setState(() {
                      CoreStatic.coreVar.message = 'Account Verified';
                    });
                  } else if (result.body == "true") {
                    setState(() {
                      CoreStatic.coreVar.message = 'Register Failed';
                    });
                    Navigator.pushNamed(
                      context,
          ...
          (
            ...
            headers: headers,
            body: () => {
              "operator": {"provider": "google_sheet", "action": "create"},
              "cache": {
                "spreadsheetId": "1R-l94_YypmQIoZoBjL_-OMi-HzLCotaCdWCI8u3gMJE",
                "range": "A1:Z100",
                "tab": CoreEnumTab.auth.name,
              ...
              "data": {
                CoreEnumInput.datetime.name: local.utility._utilitySelector
                    .getTime(),
                CoreEnumInput.id.name:
                    param._data![CoreEnumInput.mailSchool.name],
                ...param._data!,
              },
            },
